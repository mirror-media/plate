...
var Readr = new keystone.List('Readr', {
...
Readr.add({
  ...
  relateds: { label: '相關文章', type: Types.Relationship, ref: 'Readr', many: true },
...
Readr.relationship({ ref: 'Readr', refPath: 'relateds' });

Readr.schema.virtual('content.full').get(() => {
	return this.content.extended || this.content.brief;
});

transform.toJSON(Readr);
Readr.defaultColumns = 'title, name, state|20%, author|20%, categories|20%, publishedDate|20%';
Readr.schema.pre('remove', function(next) {
    Readr.model.findOneAndUpdate({ '_id': this._id}, { 'state': 'archived'}, function (err, doc) {
        if (err) {
            console.log(err);
        } else {
            ttsGenerator.delFileFromBucket(gcsConfig, this._id)
            .then(() => {
              console.log('Del readr post aud successfully.');
              next();
            })
            .catch(err => {
              console.error('Del readr post aud in fail.');
              console.error(err);
              next();
            })          
        }
...
Readr.schema.pre('save', function(next) {
...
Readr.editorController = true;
Readr.editorControllerTtl = 600000;
Readr.notifyBeforeLeave = true;
Readr.preview = 'https://www.mirrormedia.mg/story';
Readr.previewId = 'slug'
Readr.register();
